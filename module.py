...
async def user_info_command(client, message):
    try:
        # Отримання ID користувача з команди
...
@hikka.on_message(filters.command('publicchats'))
async def public_chats_command(client, message):
    try:
        # Отримання ID користувача з команди
        user_id = int(message.text.split(' ', 1)[1])
    except (IndexError, ValueError):
    ...
    try:
        ...
        await message.reply(f"Public Chats:\n{public_chats_str}")

    except Exception as e:
        await message.reply(f"Не вдалося отримати публічні чати: {str(e)}")
